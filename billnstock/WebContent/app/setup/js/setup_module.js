angular
		...
		.controller(
				...
				function($scope, $window, $mdToast, $timeout, $mdSidenav,
					...
					/*
					 * $scope.business = { businessName : "", accounttype:"",
					 * address1:"", address2:"", registerDate:"" }
					 * $scope.userEntity={ businessAccount:"", email_id : "",
					 * firstName : "", lastName : "", authority:[],
					 * isGoogleUser:true }
					 */
					...
					/*
					 * $scope.googleUser = function(ev) { // Appending dialog to
					 * document.body to cover sidenav in docs app var confirm =
					 * $mdDialog.confirm() .title('Would you like to change
					 * psssword?') .textContent('Choose Alfa-Numeric combination
					 * for safe password.') .ariaLabel('Lucky day')
					 * .targetEvent(ev) .ok('Yesssss') .cancel('No');
					 * $mdDialog.show(confirm).then(function() { $scope.status =
					 * 'You decided Yes.';
					 * 
					 * window.location =
					 * "https://www.google.com/settings/passwordchange"; },
					 * function() { $scope.status = 'You decided No.'; }); };
					 */
