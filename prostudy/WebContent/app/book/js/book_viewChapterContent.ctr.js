angular.module("prostudyApp").controller(
		...
		function($scope, $window, $mdToast, $timeout, $mdSidenav, $mdUtil,
			...
			$scope.tempChapter = {id: "",chapter_name :  "", chapter_content: "", board:"", student_class:"",subject:"",chapter_no:""};
			
			$scope.showSavedToast = function() {
				$mdToast.show($mdToast.simple().content('Chapter_Content Displayed !!!!')
						.position("top").hideDelay(3000));
			};//end of showSavedToast
			
			
			//For chapter_content, we pass its chapter.id as  selectedChapterId
			
			$log.debug("$stateParams:", $stateParams);
			$log.debug("$stateParams.selectedBookId:",$stateParams.selectedBookId);

			$log.debug("$stateParams:", $stateParams);
			$log.debug("$stateParams.selectedChapterId:",$stateParams.selectedChapterId);
			...
			$scope.showChapterContent = function() 
			{
				...
				ChapterService.getChaptersByID($scope.selectedChapterId)
						.then(function(chapterList) {
							...
							$log.debug("chapterList :"+ chapterList);
			...
			$scope.onNextChapter = function()
			{
				...
				{
				$scope.totalChapters = $scope.selectedChapter.slice(
						($scope.currentPage * $scope.itemsPerPage)- $scope.itemsPerPage,
						($scope.currentPage * $scope.itemsPerPage));
				$log.debug(" totalChapters in onNextChapter "+  angular.toJson($scope.chapterStruct ));
			...
			$scope.onPreviousChapter = function() 
			{
				...
				$scope.totalChapters = $scope.selectedChapter.slice(
						($scope.currentPage * $scope.itemsPerPage)
								- $scope.itemsPerPage,
						($scope.currentPage * $scope.itemsPerPage));
				$log.debug(" totalChapters in onPreviousChapter "+  angular.toJson($scope.chapterStruct ));
				}
				console.log("$scope.currentPage=" + $scope.currentPage);
...
/*			$scope.onNextChapter = function(currentIndex) {
var nextIndex = currentIndex+1;
if (nextIndex >= $scope.selectedChapter.length) {
...
$scope.onPreviousChapter = function(currentIndex) {
var prevIndex = currentIndex+1;
if (prevIndex < 0) {
  // move over to the last index, if we already are at the start
  prevIndex = $scope.selectedChapter.length - 1;
}
return prevIndex;
}*/
