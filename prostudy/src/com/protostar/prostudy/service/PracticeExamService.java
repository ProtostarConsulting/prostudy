...
public class PracticeExamService {
	...
	public List<PracticeExamEntity> getPracticeExamById(@Named("examId") String idvalue) {
		return ofy().load().type(PracticeExamEntity.class)
				.filter("examId", idvalue).list();
	...
	@ApiMethod(name="getPracticeExamById") 
	  public PracticeExamEntity getPracticeExamById(@Named("examId") String struct) {
		...
		PracticeExamEntity stru = ofy().load().type(PracticeExamEntity.class).filter("examId", struct).first().now();
